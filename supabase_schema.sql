...
--    - 과거 버전처럼 DROP TABLE CASCADE로 전체를 지우고 다시 만들지 않습니다.
-- ==========================================================
...
-- 3. Upsert Seed Data
-- 3a. Ingredients (식약처 고시 및 권장량/고함량 효과/부작용/대안추천 정보 적재)
...
-- 3d. Symptoms
-- symptoms는 자연키가 없어 ON CONFLICT를 쓸 수 없으므로, 이 시드가 관리하는 카테고리(1~6)의
-- 증상만 삭제 후 재삽입합니다. 관리자 패널에서 7번 이후 카테고리에 추가한 증상은 영향받지 않습니다.
...
-- 3e. Synergy Combinations (추천 시너지 조합 패키지)
...
-- 3f. Synergy Ingredients Mapping (시너지 매핑 브릿지 데이터)
-- 이 시드가 관리하는 시너지(1~7)의 매핑만 삭제 후 재삽입하여, 구성 성분이 바뀌어도 깨끗하게 갱신됩니다.
