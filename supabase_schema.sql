-- ==========================================================
-- PillSync Supabase Database Schema & Seed Script (식약처 정형 데이터 고도화)
-- ==========================================================
...
-- 3. Create Ingredients Mapping (식약처 공인 성분 및 쿠팡 매칭 + 권장량/부작용 추가)
...
-- 8. Insert Seed Data
-- 8a. Ingredients (식약처 고시 및 권장량/고함량 효과/부작용 기획 정보 적재)
INSERT INTO ingredients_mapping (id, name, fda_functional_summary, coupang_search_keyword, kfda_daily_intake, high_dose_ratio, high_dose_effect, side_effects, intake_tip) VALUES
-- 피로 개선 카테고리 성분
...
('맥주효모', '건조 맥주효모 (Brewer''s Yeast)', '단백질(아미노산 18종), 비타민 B군, 셀레늄이 풍부하여 모발 및 두피에 영양을 직접 공급하고 모발 건강을 돕는 필수 원료', '국산 맥주효모 환 대용량', 
 ...
 '모발 케라틴을 만드는 18종의 아미노산 원료를 혈액에 대량 공급하여 가늘어짐과 탈모 방어', 
...
-- 8d. Synergy Combinations (추천 시너지 조합 패키지)
...
(4, '체지방 이중 컷팅 패키지', '신규 탄수화물 유입 차단 + 기존 축적 체지방 강제 연소', 
 '가르시니아가 밥/빵 등의 신규 탄수화물이 지방으로 변환되는 길목을 차단하고, 카테킨이 에피갈로카테킨 갈레이트(EGCG) 대사 작용을 자극하여 이미 축적된 내장지방과 피하지방의 연소를 활성화시킵니다.');
...
-- 8e. Synergy Ingredients Mapping (시너지 매핑 브릿지 데이터)
